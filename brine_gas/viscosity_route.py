...
def _composition(composition=None, salts=None, m=None, S=None):
    ...
    if salts is not None:
        from brine_gas.salt_route import ions_from_salts
        return ions_from_salts(salts)
    if S is not None:
        from brine_gas.brine_properties import molality_from_salinity
...
def pressure_factor(T_K, P_MPa, composition):
    ...
    """
    from brine_gas.appelo_volumes import ionic_strength
...
def salt_ratio(T_K, P_MPa, composition=None, salts=None, m=None, S=None,
    ...
    if route == 'jones_dole':
        from brine_gas.jones_dole_viscosity import viscosity_ratio
...
def mu_water(T_K, P_MPa, route=None):
    ...
    route = route or DEFAULT_ROUTE
    if route == 'jones_dole':
        from brine_gas.jones_dole_viscosity import mu_water as _mu
        return _mu(T_K, P_MPa)
    from brine_gas.salt_viscosity_benchmark import mu_water_maoduan
...
if __name__ == '__main__':
    print("Brine viscosity dispatcher - default route:", DEFAULT_ROUTE)

    print("\nSalinity with no species named is NaCl, on every input form:")
    for kw in ({'m': 3.0}, {'salts': {'NaCl': 3.0}},
               {'composition': {'Na+': 3.0, 'Cl-': 3.0}}):
        print(f"   {str(kw):<48} "
              f"{brine_viscosity(373.15, 30.0, **kw):.5f} mPa s")
    print(f"   {'S = 0.1495 weight fraction':<48} "
          f"{brine_viscosity(373.15, 30.0, S=0.14943):.5f} mPa s")

    print("\nMulti-salt (only the default route can do this):")
    for salts in ({'NaCl': 3.0, 'CaCl2': 1.0},
                  {'NaCl': 2.0, 'KCl': 0.3, 'MgCl2': 0.8, 'CaCl2': 0.5},
                  {'KCl': 4.0}):
        print(f"   {str(salts):<52} "
              f"{brine_viscosity(373.15, 30.0, salts=salts):.5f} mPa s")

    print("\nThe superseded route refuses what it cannot represent:")
    try:
        brine_viscosity(373.15, 30.0, salts={'KCl': 4.0}, route='mao_duan')
    except ValueError as e:
        print(f"   ValueError: {str(e)[:100]}...")

    print("\nWhat the pressure factor is worth, 3 molal NaCl:")
    for T_C in (20.0, 60.0, 150.0):
        f = pressure_factor(T_C + 273.15, 35.0, {'Na+': 3.0, 'Cl-': 3.0})
        print(f"   {T_C:5.0f} degC, 35 MPa: ratio x {f:.4f} "
              f"({100 * (f - 1):+.2f}%)")
