...
# ============================================================================
# Region 4 saturation line (needed to reject the vapour region)
# ============================================================================

T_MIN = 273.15   # K, Region 1 lower temperature bound
T_MAX = 623.15   # K, Region 1 upper temperature bound
P_MAX = 100.0    # MPa, Region 1 upper pressure bound

# IAPWS-IF97 Region 4 basic equation, Table 34
_REGION4_N = [
...
def p_sat_if97(T):
    ...
    Valid 273.15 K to 647.096 K. Below this pressure at a given T the stable
    phase is vapour, where Region 1 must not be evaluated.
    """
    n = _REGION4_N
    theta = T + n[8] / (T - n[9])
    ...
    return (2.0 * C / (-B + np.sqrt(B * B - 4.0 * A * C))) ** 4


def _check_region1(T, P):
    """
    Validate that (T, P) lies inside IAPWS-IF97 Region 1 (compressed liquid).

    Region 1 is bounded by 273.15-623.15 K, the saturation line below and
    100 MPa above. Evaluating it outside the T and P bounds returns a plausible
    but meaningless number, so those are enforced here.

    The saturation line is NOT enforced: Region 1 extrapolates smoothly into
    the metastable liquid, and brine correlations rely on that to obtain an
    atmospheric-reference density at reservoir temperature. The check that
    matters - V2inf is undefined when the solvent is vapour - is applied in
    plyasunov_model.V2_inf via p_sat_if97.
    """
    if not T_MIN <= T <= T_MAX:
        raise ValueError(
            f"Temperature {T} K is outside IAPWS-IF97 Region 1 bounds "
            f"({T_MIN}-{T_MAX} K)"
        )
    if P <= 0 or P > P_MAX:
        raise ValueError(
            f"Pressure {P} MPa is outside IAPWS-IF97 Region 1 bounds. "
            f"The correlation is valid only to the Region 1 limit of "
            f"{P_MAX:.0f} MPa (14,503 psia); supply a lower pressure."
        )
