...
import numpy as np
from brine_gas.water_properties import rho_w, kappa_T, TC_WATER, MW_WATER
...
def V2_inf(gas, T, P):
    ...
    Raises if P is below the water saturation pressure at T: V2∞ describes a
    gas dissolved in LIQUID water, so a vapour solvent makes it undefined.
    Region 1 itself extrapolates into the metastable liquid and is not bounded
    at the saturation line, so the check belongs here rather than in rho_w.
    """
    p_s = p_sat_if97(T)
    if P < p_s:
        raise ValueError(
