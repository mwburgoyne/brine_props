...
# Single source of truth: salinity_from_molality and the Spivey wt-fraction to
# molality conversion previously disagreed (58.44 vs 58.4428), so the m -> S -> m
# round trip was not exact.
M_NACL = 58.4428  # g/mol
...
def molality_from_salinity(S):
    """NaCl molality (mol/kg water) from weight fraction. Inverse of the above."""
    S = float(S)
    if S <= 0.0:
        return 0.0
