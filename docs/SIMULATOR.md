...
- NaCl needs coefficients of its own because the reference is pure water: the simulator computes brine viscosity as its water viscosity times $10^{B_{\mathrm{NaCl}} w_{\mathrm{NaCl}}}$, with the gas factors on top. The salt ratio is not linear in weight fraction, which the form assumes, so the set is a least-squares fit through the origin over 0.5-5 mol/kg: it reproduces the baseline to within 4% and scores 1.8% mean, 6.1% worst against Kestin's NaCl measurements (20-150 degC, 0.1-35 MPa). The tNavigator default set (0.718, 0.00359, 0) scores 3.7% mean and 20% worst, better than ours only below 1.5 mol/kg (`python fits/ezrokhi_fits.py --check`). The table quotes the 30 MPa set; pressure moves the salt ratio by up to 3% of viscosity, so Table A.3 below gives the $b_j$ as cubics in pressure, within 0.3% of viscosity at 5 mol/kg.
